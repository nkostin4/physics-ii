...
// draw surface
triple F(pair uv) {
    return (2.0*sin(uv.x)*cos(uv.y),2.0*sin(uv.x)*sin(uv.y),1.7*cos(uv.x));
}

surface balloon = surface(F, (0,0), (5*pi/6,2*pi), Spline);
draw(shift(5,0,1.0)*balloon, surfacepen=blue+opacity(0.3));
